...
function ServiceStopper(){
    ...
    $running = (get-service | findstr Running)
	$success =@()
	$fail =@()
    ...
    PrintFormatting('Running Services')
  #  foreach($zz in $running){
        PrintFormatting($running)
  #  }
...
function ProcessStopper(){
    ...
    $running =(Get-Process | select-object Name)
	$success =@()
	$fail =@()
    ...
    PrintFormatting('Running Processes')
   # foreach($zz in $running){
        PrintFormatting($running)
   # }
	PrintFormatting('Succesfully closed:')
